...
# Control de sesión para autenticación en Pestaña 2
...
if not st.session_state.autenticado_consulta:
    st.markdown('<p class="main-header">Acceso Restringido - Módulo de Consulta</p>', unsafe_allow_html=True)
    st.markdown('<p class="sub-header">Ingrese sus credenciales operativas</p>', unsafe_allow_html=True)
    
    with st.form("form_login_consulta"):
        usuario = st.selectbox("Seleccione Usuario:", options=["Seleccione...", "Operativo"])
    ...
    # Contenido principal de la Pestaña 2 una vez autenticado
    ...
    if "registros_censales" not in st.session_state or not st.session_state.registros_censales:
        st.info("No hay pacientes registrados en la sesión actual.")
    else:
